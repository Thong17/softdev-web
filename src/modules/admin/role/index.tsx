    ...
    { 
      device === 'mobile' 
        ? ( privilege?.role?.detail && <MenuDialog label={<ViewButton />}>
            <MenuList component='div' onClick={() => navigate(`/admin/role/update/${id}`)}>Edit</MenuList>
            <MenuList component='div' onClick={() => setDialog({ open: true, id })}>Delete</MenuList>
            <MenuList component='div' onClick={() => navigate(`/admin/role/detail/${id}`)}>View</MenuList>
          </MenuDialog> ) 
        : <>
...
export const Roles = () => {
  ...
  useEffect(() => {
    const listRoles = roles.map((role: any) => {
      return createData(role._id, role.name?.[lang] || role.name?.['English'], role.description || '...', role.createdBy || '...', user?.privilege, device, navigate, setDialog)
    })
